...
(function ($) {
    ...
    app.views.CheckProductsCreate = Backbone.View.extend({
        ...
        events: {
            ...
            'click #PrintFile': 'PrintFile',
            //'click .cancelClick': 'cancelClick',
            'change #checkingDateBox': 'checkingDateBoxChange',
        ...
        initialize: function () {
            ...
            this.$el.find('#checkingDateBox').datebox({
                mode: "calbox",
                useFocus: true,
            ...
            var dateStr = this.dateStr = this.$el.find("#checkingDateBox").val();//new Date().toISOString().slice(0, 10);

            var exportProductTable = this.$el.find('#exportProductTable').get(0);
            ...
            var greenRenderer = function (instance, td, row, col, prop, value, cellProperties) {
                ...
                td.style.backgroundColor = '#89ff5a';
        ...
        PrintFile: function () {
            ...
            var detailStr = 'Checking for: ' + this.dateStr.replace(/-/g, '/') +
                 ' : ' + stockSelected;

            var myWindow = window.open("../report/CheckProductsReport.html", 'test', 'width=' + 900 + ',height=' + 500 + '');
        ...
        checkingDateBoxChange: function () {
            var $time = this.$el.find("#checkingDateBox");
            //var time = addTimezoneOffset(new Date($time.val()));
            var dateStr = this.dateStr = $time.val();// time.toISOString().slice(0, 10);

            this.collection.each(function (model) {
                model.set('check_date', dateStr);
            });
        }
    });
